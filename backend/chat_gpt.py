# -*- coding: utf-8 -*-
"""
Сценарий диалога с генеративной системой.
"""
...
load_dotenv()
SECRET_KEY = os.getenv('SECRET_KEY', default='ваш ключ в VseGPT после регистрации')

import openai


def chat_forecast(message: str, key: str=SECRET_KEY) -> str:
    '''Логика педсказания'''
    openai.api_key  = key
    #openai.api_key
    ...
    prompt = """
    Ты модератор неофициальных обращений граждан в России.
    Твоя задача - классифицировать обращения и вопросы граждан.
    Относи обращение к конкретной ТЕМЕ, к конкретной ГРУППЕ ТЕМ. Выдавай ответ СПИСКОМ.
    И БОЛЬШЕ НИЧЕГО НЕ ГОВОРИ, НЕ ПИШИ, НЕ ПРЕДЛАГАЙ! НИ В КОЕМ СЛУЧАЕ НЕ ПРЕДЛАГАЙ СПОСОБЫ РЕШЕНИЯ ПРОБЛЕМЫ.
    Ты должен выдавать ТОЛЬКО одно значение из этого списка наименований Исполнителей:
